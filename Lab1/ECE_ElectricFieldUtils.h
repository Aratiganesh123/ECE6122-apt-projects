...
template <typename T>
void getInput(const std::string& prompt, std::vector<T>& values, int expectedNumber, const std::string& error,  bool (*validationFunction)(const Point3D&, const std::vector<Point3D>&), const std::vector<Point3D>& gridPoints)
{
    bool endLoop = false;
    do
    {
